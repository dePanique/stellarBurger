...
const BurgerIngredients = () => {
  ...
  useEffect(() => {
    ...
    setPositionForActivation(offsets.map((_, index) => {
      if (index === 0) {
        return 0;
      }
      else if (index === 1) {
  ...
  const setCurrentTab = (e) => {
    ...
    if (currentScrollPos < positionForActivation[1]) {
      setActiveTab('Булки')
    } else if (currentScrollPos > positionForActivation[1] && currentScrollPos < positionForActivation[2]) {
      setActiveTab('Соусы')
    ...
    <section className={styles.burgerIngredients + " mr-5"}>
      ...
      <div className={styles.tabRow + " mb-10"}>
        <a href="#bun" className={styles.link + " text text_type_main-default"}>
          <Tab value="Булки" active={activeTab === "Булки"} onClick={setActiveTab}>
            Булки
          </Tab>
        </a>
        <a
        ...
        >
          <Tab value="Соусы" active={activeTab === "Соусы"} onClick={setActiveTab}>
            Соусы
          </Tab>
        </a>
        <a
          ...
          className={styles.link + " text text_type_main-default"}
        >
          <Tab value="Начинки" active={activeTab === "Начинки"} onClick={setActiveTab} >
            Начинки
          </Tab>
        </a>
      </div>
